...
**M1 に必要な6本は 2026-08-10 にレビューを完了し、`reviewed`（充実レベル）へ進めた。**
レビューで解消した内容は下記のとおり。

| 指摘 | 対応 |
|---|---|
| フェーズ削除の禁止と、時間ログ削除仕様の不在が矛盾 | 警告つきで削除を許す形に変更（`FR-HIER-001.340` / `.350`） |
| 階層の深さ10段の制約に根拠がない | 上限を撤回。移植時に移植元を確認する事項として記録 |
| 否定表現の裏に条件分岐が隠れている（2件） | else を明示（`FR-HIER-001.430` / `FR-SUM-001.120`） |

### M1 に必要な要求

| ID | 表題 | ユニット | 状態 |
|---|---|---|---|
| `FR-HIER-001` | 階層を構成してプロセスを割り当てる | B-2, B-3 | 🟢 `reviewed` |
| `FR-TIME-001` | 作業時間を計測して時間ログに記録する | B-4 | 🟢 `reviewed` |
| `FR-SUM-001` | 記録した作業時間を集計して表示する | C | 🟢 `reviewed` |
| `NFR-I18N-001` | 英語と日本語で表示を切り替える | C | 🟢 `reviewed` |
| `NFR-DATA-001` | 記録したデータを永続化して再起動後も参照できるようにする | B-9 | 🟢 `reviewed` |
| `CON-LICENSE-001` | GPLv3 を維持して移植元の帰属を保持する | C | 🟢 `reviewed` |
